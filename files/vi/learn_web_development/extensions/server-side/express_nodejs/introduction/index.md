...
Các tác vụ phát triển web phổ biến khác không được Node hỗ trợ trực tiếp. Nếu bạn muốn thêm xử lý cụ thể cho các phương thức HTTP khác nhau (ví dụ: `GET`, `POST`, `DELETE`, v.v.), xử lý riêng các yêu cầu tại các đường dẫn URL khác nhau ("tuyến đường"), phục vụ các tệp tĩnh hoặc sử dụng mẫu để tạo động phản hồi, Node sẽ không hữu ích lắm khi tự mình thực hiện. Bạn cần tự viết mã, hoặc bạn có thể tránh tái phát minh bánh xe và sử dụng một framework web!
...
Express cung cấp các phương thức để chỉ định hàm nào được gọi cho một phương thức HTTP cụ thể (`GET`, `POST`, `PUT`, v.v.) và mẫu URL ("Tuyến đường"), và các phương thức để chỉ định công cụ mẫu ("view") nào được sử dụng, nơi các tệp mẫu được đặt và mẫu nào để render phản hồi. Bạn có thể sử dụng middleware Express để thêm hỗ trợ cho cookie, phiên và người dùng, lấy tham số `POST`/`GET`, v.v. Bạn có thể sử dụng bất kỳ cơ chế cơ sở dữ liệu nào được Node hỗ trợ (Express không định nghĩa bất kỳ hành vi liên quan đến cơ sở dữ liệu nào).
...
Trong ví dụ _Hello World_ Express của chúng tôi (xem ở trên), chúng tôi đã định nghĩa một hàm trình xử lý tuyến đường (hàm gọi lại) cho các yêu cầu HTTP `GET` đến gốc trang web (`'/'`).
...
Đối tượng _ứng dụng Express_ cũng cung cấp các phương thức để định nghĩa các trình xử lý tuyến đường cho tất cả các phương thức HTTP khác, hầu hết được sử dụng theo cách hoàn toàn giống nhau:
...
Tuyến đường cho phép bạn khớp các mẫu ký tự cụ thể trong URL và trích xuất một số giá trị từ URL và truyền chúng dưới dạng tham số cho trình xử lý tuyến đường (như các thuộc tính của đối tượng yêu cầu được truyền dưới dạng tham số).

Thường hữu ích khi nhóm các trình xử lý tuyến đường cho một phần cụ thể của trang web lại với nhau và truy cập chúng bằng cách sử dụng tiền tố tuyến đường chung (ví dụ: một trang web có Wiki có thể có tất cả các tuyến đường liên quan đến wiki trong một tệp và có chúng được truy cập với tiền tố tuyến đường _/wiki/_). Trong _Express_ điều này được thực hiện bằng cách sử dụng đối tượng [`express.Router`](https://expressjs.com/en/guide/routing.html#express-router). Ví dụ: chúng tôi có thể tạo tuyến đường wiki của mình trong một module có tên **wiki.js** và sau đó export đối tượng `Router`, như được hiển thị bên dưới:
...
> [!NOTE]
> Thêm tuyến đường vào đối tượng `Router` giống hệt như thêm tuyến đường vào đối tượng `app` (như được hiển thị trước đó).

Để sử dụng bộ định tuyến trong tệp ứng dụng chính của chúng tôi, chúng tôi sẽ `require()` module tuyến đường (**wiki.js**) và sau đó gọi `use()` trên ứng dụng _Express_ để thêm Bộ định tuyến vào đường dẫn xử lý middleware. Hai tuyến đường sau đó sẽ có thể truy cập từ `/wiki/` và `/wiki/about/`.
...
Chúng tôi sẽ chỉ cho bạn nhiều hơn về làm việc với tuyến đường, và đặc biệt là về việc sử dụng `Router`, sau này trong phần được liên kết [Tuyến đường và bộ điều khiển](/en-US/docs/Learn_web_development/Extensions/Server-side/Express_Nodejs/routes).

### Sử dụng middleware

Middleware được sử dụng rộng rãi trong các ứng dụng Express, cho các tác vụ từ phục vụ các tệp tĩnh đến xử lý lỗi, đến nén các phản hồi HTTP. Trong khi các hàm tuyến đường kết thúc chu kỳ yêu cầu-phản hồi HTTP bằng cách trả về một số phản hồi cho máy khách HTTP, các hàm middleware _thường_ thực hiện một số hoạt động trên yêu cầu hoặc phản hồi và sau đó gọi hàm tiếp theo trong "ngăn xếp", có thể là nhiều middleware hơn hoặc một trình xử lý tuyến đường. Thứ tự gọi middleware là tùy thuộc vào nhà phát triển ứng dụng.
...
> [!NOTE]
> Middleware và các hàm định tuyến được gọi theo thứ tự chúng được khai báo. Đối với một số middleware, thứ tự là quan trọng (ví dụ: nếu session middleware phụ thuộc vào cookie middleware, thì cookie handler phải được thêm đầu tiên). Hầu như luôn là trường hợp middleware được gọi trước khi thiết lập các tuyến đường, hoặc các trình xử lý tuyến đường của bạn sẽ không có quyền truy cập vào chức năng được thêm bởi middleware của bạn.

Bạn có thể viết các hàm middleware của riêng mình và bạn có khả năng phải làm vậy (ít nhất là để tạo mã xử lý lỗi). Sự **khác biệt duy nhất** giữa hàm middleware và hàm gọi lại trình xử lý tuyến đường là các hàm middleware có đối số thứ ba `next`, mà các hàm middleware được mong đợi gọi nếu chúng không phải là thứ hoàn thành chu kỳ yêu cầu (khi hàm middleware được gọi, điều này chứa hàm _next_ phải được gọi).

Bạn có thể thêm hàm middleware vào chuỗi xử lý cho _tất cả các phản hồi_ với `app.use()`, hoặc cho một phương thức HTTP cụ thể bằng cách sử dụng phương thức liên quan: `app.get()`, `app.post()`, v.v. Tuyến đường được chỉ định theo cùng một cách cho cả hai trường hợp, mặc dù tuyến đường là tùy chọn khi gọi `app.use()`.

Ví dụ dưới đây hiển thị cách bạn có thể thêm hàm middleware bằng cả hai phương pháp, và có/không có tuyến đường.
...
> [!NOTE]
> Ở trên chúng tôi khai báo hàm middleware riêng biệt và sau đó đặt nó làm hàm gọi lại. Trong hàm trình xử lý tuyến đường trước đó của chúng tôi, chúng tôi đã khai báo hàm gọi lại khi nó được sử dụng. Trong JavaScript, cả hai cách tiếp cận đều hợp lệ.
...
Chúng có thể trả về bất kỳ nội dung nào được yêu cầu, nhưng phải được gọi sau tất cả các lệnh gọi `app.use()` và tuyến đường khác để chúng là middleware cuối cùng trong quá trình xử lý yêu cầu!
...
Xin chúc mừng, bạn đã hoàn thành bước đầu tiên trong hành trình Express/Node của mình! Bây giờ bạn nên hiểu những lợi ích chính của Express và Node, và đại khái những phần chính của ứng dụng Express có thể trông như thế nào (tuyến đường, middleware, xử lý lỗi và mã mẫu). Bạn cũng nên hiểu rằng với Express là một framework không áp đặt quan điểm, cách bạn ghép các phần này lại với nhau và các thư viện bạn sử dụng phần lớn tùy thuộc vào bạn!
