...
> [!NOTE]
> Chúng ta chưa xem xét cách hạn chế các route cụ thể đối với người dùng được xác thực hoặc ủy quyền, vì vậy tại thời điểm này, bất kỳ người dùng nào cũng có thể thực hiện thay đổi trong cơ sở dữ liệu.
...
Xử lý biểu mẫu sử dụng tất cả các kỹ thuật tương tự mà chúng ta đã học để hiển thị thông tin về các model của mình: route gửi yêu cầu của chúng ta đến một hàm bộ điều khiển thực hiện bất kỳ hành động cơ sở dữ liệu nào được yêu cầu, bao gồm đọc dữ liệu từ các model, sau đó tạo và trả về một trang HTML. Điều làm cho mọi thứ phức tạp hơn là máy chủ cũng cần có khả năng xử lý dữ liệu do người dùng cung cấp, và hiển thị lại biểu mẫu với thông tin lỗi nếu có bất kỳ vấn đề nào.
...
Thường thì mã xử lý biểu mẫu được triển khai bằng một route `GET` để hiển thị biểu mẫu ban đầu và một route `POST` đến cùng đường dẫn để xử lý kiểm tra hợp lệ và xử lý dữ liệu biểu mẫu. Đây là cách tiếp cận sẽ được sử dụng trong hướng dẫn này.
...
Các chuỗi kiểm tra hợp lệ và làm sạch là middleware nên được truyền vào trình xử lý route Express (chúng ta thực hiện điều này gián tiếp, thông qua bộ điều khiển). Khi middleware chạy, mỗi validator/sanitizer được chạy theo thứ tự được chỉ định.
...
Để triển khai mã xử lý biểu mẫu của chúng ta, chúng ta sẽ cần hai route có cùng mẫu URL. Route đầu tiên (`GET`) được sử dụng để hiển thị một biểu mẫu rỗng mới để tạo đối tượng. Route thứ hai (`POST`) được sử dụng để kiểm tra hợp lệ dữ liệu do người dùng nhập, và sau đó lưu thông tin và chuyển hướng đến trang chi tiết (nếu dữ liệu hợp lệ) hoặc hiển thị lại biểu mẫu với lỗi (nếu dữ liệu không hợp lệ).

Chúng ta đã tạo các route cho tất cả các trang tạo model của mình trong **/routes/catalog.js** (trong [hướng dẫn trước](/en-US/docs/Learn_web_development/Extensions/Server-side/Express_Nodejs/routes)). Ví dụ, các route thể loại được hiển thị dưới đây:
