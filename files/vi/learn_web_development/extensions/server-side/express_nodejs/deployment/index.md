...
Mở tệp **./app.js** và yêu cầu thư viện compression như được hiển thị. Thêm thư viện compression vào chuỗi middleware bằng phương thức `use()` (điều này nên xuất hiện trước bất kỳ route nào bạn muốn nén — trong trường hợp này, tất cả chúng!)
...
### Thêm giới hạn tốc độ cho các route API
...
Gói này có thể được sử dụng để giới hạn số lượng yêu cầu có thể được thực hiện đến một route cụ thể hoặc tập hợp route.
...
Bây giờ chạy trang web (xem [Kiểm thử các route](/en-US/docs/Learn_web_development/Extensions/Server-side/Express_Nodejs/routes#testing_the_routes) cho các lệnh có liên quan) và kiểm tra xem trang web vẫn hoạt động như bạn mong đợi.
