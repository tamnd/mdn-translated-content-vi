...
Tiêu đề HTTP **`Want-Content-Digest`** {{glossary("request header", "request")}} và {{glossary("response header")}} cho biết sở thích để người nhận gửi tiêu đề toàn vẹn {{HTTPHeader("Content-Digest")}} trong các thông điệp liên quan đến URI yêu cầu và siêu dữ liệu biểu diễn.
...
- `<algorithm>`
  - : Thuật toán được yêu cầu để tạo digest của nội dung thông điệp.
...
- {{HTTPHeader("Content-Digest")}}, {{HTTPHeader("Repr-Digest")}}, {{HTTPHeader("Want-Repr-Digest")}} tiêu đề digest
