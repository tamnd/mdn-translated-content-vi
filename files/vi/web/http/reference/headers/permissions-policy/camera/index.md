---
title: "Permissions-Policy: chỉ thị camera"
short-title: camera
...
Chỉ thị `camera` của header HTTP {{HTTPHeader("Permissions-Policy")}} kiểm soát việc tài liệu hiện tại có được phép sử dụng thiết bị đầu vào video hay không.
...
Danh sách cho phép mặc định cho `camera` là `self`.
