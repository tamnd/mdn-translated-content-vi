...
Thuộc tính [CSS](/en-US/docs/Web/CSS) **`inset-block`** định nghĩa các offset đầu và cuối khối logic của một phần tử, ánh xạ đến các offset vật lý tùy thuộc vào chế độ viết, hướng và hướng văn bản của phần tử. Nó tương ứng với các thuộc tính {{cssxref("top")}} và {{cssxref("bottom")}}, hoặc {{cssxref("right")}} và {{cssxref("left")}} tùy thuộc vào các giá trị được định nghĩa cho {{cssxref("writing-mode")}}, {{cssxref("direction")}} và {{cssxref("text-orientation")}}.
...
Thuộc tính `inset-block` nhận các giá trị giống như thuộc tính {{cssxref("top")}}.
...
- Thuộc tính viết tắt inline được ánh xạ: {{cssxref("inset-inline")}}
