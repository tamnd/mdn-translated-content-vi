...
Thuộc tính **`grid-column`** [CSS](/en-US/docs/Web/CSS) là [thuộc tính viết tắt](/en-US/docs/Web/CSS/Guides/Cascade/Shorthand_properties) dùng để chỉ định kích thước và vị trí của một phần tử lưới bên trong {{glossary("grid column", "cột lưới")}} bằng cách đóng góp một đường, một khoảng mở rộng (span), hoặc không có gì (tự động) vào vị trí lưới của nó, từ đó xác định cạnh bắt đầu và kết thúc theo chiều inline của {{glossary("grid areas", "vùng lưới")}} tương ứng.
...
Nếu hai giá trị `<grid-line>` được cho, chúng được phân tách bởi `/`.
Thuộc tính thành phần `grid-column-start` được đặt thành giá trị trước dấu gạch chéo, và thuộc tính thành phần `grid-column-end` được đặt thành giá trị sau dấu gạch chéo.
...
- `<custom-ident>`
  - : Nếu có một đường được đặt tên với tên `<custom-ident>-start`/`<custom-ident>-end`, nó đóng góp đường đầu tiên như vậy vào vị trí của phần tử lưới.
    ...
    > [!NOTE]
    > Các vùng lưới được đặt tên tự động tạo ra các đường được đặt tên ẩn theo dạng này, vì vậy việc chỉ định `grid-column: foo;` sẽ chọn cạnh bắt đầu/kết thúc của vùng lưới được đặt tên đó (trừ khi một đường khác có tên `foo-start`/`foo-end` được chỉ định rõ ràng trước đó).
...
### Đặt kích thước và vị trí cột lưới
