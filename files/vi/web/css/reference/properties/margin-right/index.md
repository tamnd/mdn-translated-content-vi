...
Thuộc tính **`margin-right`** [CSS](/en-US/docs/Web/CSS) đặt [vùng lề](/en-US/docs/Web/CSS/Guides/Box_model/Introduction#margin_area) ở phía bên phải của phần tử. Giá trị dương đặt phần tử xa hơn so với các phần tử lân cận, trong khi giá trị âm đặt nó gần hơn.
...
- `auto`
  - : Lề phải nhận một phần không gian ngang chưa sử dụng, được xác định chủ yếu bởi chế độ bố cục được dùng. Nếu cả giá trị của `margin-left` và `margin-right` đều là `auto`, không gian tính toán được phân bổ đều nhau. Bảng dưới đây tóm tắt các trường hợp khác nhau:
...
- {{cssxref("margin-top")}}, {{cssxref("margin-bottom")}}, và {{cssxref("margin-left")}}
