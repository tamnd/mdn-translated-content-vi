...
Thuộc tính [CSS](/en-US/docs/Web/CSS) **`corner-bottom-shape`** chỉ định hình dạng của cả hai góc trên cạnh dưới của một hộp, trong phạm vi vùng {{cssxref("border-radius")}} của chúng.
...
Thuộc tính `corner-bottom-shape` là viết tắt của các thuộc tính vật lý sau:
...
Thuộc tính `corner-bottom-shape` được chỉ định bằng một hoặc hai giá trị {{cssxref("&lt;corner-shape-value>")}}:

- Nếu dùng **một giá trị**, nó chỉ định hình dạng của **cả hai góc dưới**.
- Nếu dùng **hai giá trị**, giá trị đầu tiên chỉ định hình dạng của góc **dưới-trái**, và giá trị thứ hai chỉ định hình dạng của góc **dưới-phải**.
...
### Cách dùng cơ bản của `corner-bottom-shape`
...
Chúng ta cho hộp một số kiểu dáng cơ bản, được ẩn để ngắn gọn. Chúng ta cũng áp dụng {{cssxref("box-shadow")}}, `border-radius` là `60px 30px 20% 40%`, và `corner-bottom-shape` là `square squircle`.
