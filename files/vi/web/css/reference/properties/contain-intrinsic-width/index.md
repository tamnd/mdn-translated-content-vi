...
Thuộc tính CSS **`contain-intrinsic-width`** đặt chiều rộng của một phần tử mà trình duyệt sẽ dùng cho bố cục khi phần tử đó bị ràng buộc bởi [size containment](/en-US/docs/Web/CSS/Guides/Containment/Using#size_containment).
...
- `<length>`
  - : Phần tử có chiều rộng được chỉ định ({{cssxref("&lt;length&gt;")}}).
- `auto <length>`
...
Thuộc tính `contain-intrinsic-width` cho phép tác giả chỉ định một giá trị phù hợp để dùng làm chiều rộng cho bố cục.

Giá trị `auto <length>` cho phép chiều rộng của phần tử được lưu trữ nếu phần tử đó từng được "hiển thị bình thường" (với các phần tử con của nó), sau đó được dùng thay vì chiều rộng được chỉ định khi phần tử đang bỏ qua nội dung.
