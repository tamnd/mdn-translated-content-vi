...
Thuộc tính `scroll-padding-block-end` xác định các độ lệch cho cạnh cuối trong chiều block của _vùng hiển thị tối ưu_ của scrollport: vùng được sử dụng làm vùng mục tiêu để đưa các thứ vào tầm nhìn của người dùng. Điều này cho phép tác giả loại trừ các vùng của scrollport bị che khuất bởi nội dung khác (chẳng hạn như thanh công cụ hoặc thanh bên cố định) hoặc tạo thêm khoảng trống giữa phần tử mục tiêu và các cạnh của scrollport.
...
- `<length-percentage>`
  - : Độ lệch hướng vào trong từ cạnh cuối block của scrollport, dưới dạng độ dài hợp lệ hoặc phần trăm.
- `auto`
