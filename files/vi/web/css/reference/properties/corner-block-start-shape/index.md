...
Thuộc tính [CSS](/en-US/docs/Web/CSS) **`corner-block-start-shape`** chỉ định hình dạng của cả hai góc ở cạnh block-start của một hộp, trong phạm vi vùng {{cssxref("border-radius")}} của chúng.
...
Thuộc tính `corner-block-start-shape` được chỉ định bằng một hoặc hai giá trị {{cssxref("&lt;corner-shape-value>")}}:
...
Chúng ta cho hộp một số kiểu dáng cơ bản, được ẩn để ngắn gọn. Chúng ta cũng áp dụng {{cssxref("box-shadow")}}, `border-radius` là `40px 60px`, và `corner-block-start-shape` là `scoop notch`.
