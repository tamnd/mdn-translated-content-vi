...
**_Thuộc tính_** [CSS](/en-US/docs/Web/CSS) là một tham số được dùng trong [khai báo CSS](/en-US/docs/Web/CSS/Guides/Syntax/Introduction#css_declarations) cho phép bạn tạo kiểu cho một số khía cạnh nhất định của các phần tử được chọn.
Ví dụ, thuộc tính {{CSSxRef("opacity")}} được dùng để đặt độ mờ của phần tử được chọn, cho phép bạn kiểm soát xem nội dung đằng sau phần tử đó có hiển thị hay không:
...
Mỗi thuộc tính có một tên (ví dụ: `opacity`), một giá trị (ví dụ: `0.8`), và một hành vi xác định trên việc hiển thị tài liệu.
CSS cũng xác định các thuộc tính tốc ký, do đó bạn có thể chỉ định nhiều thuộc tính liên quan trong một khai báo.
Ví dụ, thuộc tính {{CSSxRef("margin")}} là tốc ký cho {{CSSxRef("margin-top")}}, {{CSSxRef("margin-right")}}, {{CSSxRef("margin-bottom")}}, và {{CSSxRef("margin-left")}}, đặt lề cho cả bốn phía của phần tử:
...
## Chỉ mục các thuộc tính theo thứ tự bảng chữ cái

Các thuộc tính tiêu chuẩn được xác định bởi các thông số kỹ thuật CSS bao gồm những mục sau:

### Khác

- [Thuộc tính tùy chỉnh (`--*`): CSS variables](/en-US/docs/Web/CSS/Reference/Properties/--*)
...
Các thuộc tính không chuẩn có tiền tố nhà cung cấp bao gồm:
...
- [Lớp giả phần tử](/en-US/docs/Web/CSS/Reference/Selectors/Pseudo-elements)
- [Các phần mở rộng CSS của Firefox có tiền tố nhà cung cấp (-moz-)](/en-US/docs/Web/CSS/Reference/Mozilla_extensions)
- [Các phần mở rộng CSS của WebKit có tiền tố nhà cung cấp (-webkit-)](/en-US/docs/Web/CSS/Reference/Webkit_extensions)
