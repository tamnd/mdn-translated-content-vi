...
Kiểu dữ liệu **`<named-color>`** [CSS](/vi/docs/Web/CSS) là tên của một màu sắc, chẳng hạn như `red`, `blue`, `black`, hoặc `lightseagreen`. Về mặt cú pháp, `<named-color>` là một {{cssxref("ident")}}.

Giá trị `<named-color>` có thể được dùng ở bất kỳ đâu mà {{cssxref("&lt;color&gt;")}} có thể được dùng.
...
Màu sắc được đặt tên bao gồm các màu tiêu chuẩn, từ khóa [`transparent`](#transparent) và [`currentColor`](/vi/docs/Web/CSS/Reference/Values/color_value#currentcolor_keyword).
...
<table>
  <thead>
    <tr>
      ...
      <th scope="col">Giá trị hex RGB</th>
      <th scope="col">Mẫu</th>
    </tr>
  </thead>
  <tbody>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Xem <a href="#transparent">transparent</a>.</td>
...
Ban đầu, trong [CSS Level 1](https://www.w3.org/TR/CSS1/#color-units), chỉ có 16 màu cơ bản được định nghĩa, với `orange` được thêm vào trong [CSS Level 2](https://www.w3.org/TR/CSS2/syndata.html#value-def-color). Các nhà thiết kế web thấy danh sách này quá ngắn, và các nhà cung cấp trình duyệt đã thêm nhiều tên màu sắc dựa trên tên màu X11. Trong [SVG 1](https://www.w3.org/TR/SVG11/types.html#DataTypeColor), rồi trong [CSS Colors Level 3](https://drafts.csswg.org/css-color-3/#svg-color), các tên này được chuẩn hóa, định nghĩa chính thức và thống nhất (một số có cách đánh vần khác nhau hiện là các bí danh). Chúng được gọi là _từ khóa màu mở rộng_, _màu X11_, hoặc _màu SVG_.

Trong [CSS Colors Level 4](https://drafts.csswg.org/css-color-4/#named-colors), một màu bổ sung, `rebeccapurple` được thêm vào để vinh danh [tiên phong web Eric Meyer](https://meyerweb.com/eric/thoughts/2014/06/19/rebeccapurple/).
...
Để tránh hành vi không mong muốn, chẳng hạn như trong {{cssxref("gradient")}}, thông số CSS hiện tại quy định rằng `transparent` phải được tính trong [không gian màu alpha-premultiplied](https://drafts.csswg.org/css-color/#interpolation-alpha). Tuy nhiên, lưu ý rằng các trình duyệt cũ hơn có thể xử lý nó như màu đen với giá trị alpha là `0`.

Từ khóa `transparent` không phải là màu thực trong CSS Level 2 (Revision 1). Đây là từ khóa đặc biệt có thể được dùng thay cho giá trị `<color>` thông thường trên hai thuộc tính CSS: {{Cssxref("background")}} và {{Cssxref("border")}}. Về cơ bản nó được thêm vào để cho phép các nhà phát triển ghi đè một màu đặc (solid) được kế thừa. Với sự ra đời của các kênh alpha trong CSS Colors Level 3, `transparent` đã được định nghĩa lại là màu thực. Nó có thể được dùng ở bất kỳ đâu mà giá trị `<color>` có thể được dùng.

## Mô tả

Tất cả tên đều chỉ định một màu trong [không gian màu sRGB](https://en.wikipedia.org/wiki/SRGB). Mặc dù các tên mô tả ít nhiều màu sắc tương ứng của chúng, nhưng về cơ bản chúng là nhân tạo, không có lý do chặt chẽ đằng sau các thuật ngữ được dùng.
...
Mặc dù nhiều từ khóa được điều chỉnh từ [X11](https://en.wikipedia.org/wiki/X_Window_System), giá trị RGB của chúng có thể khác với màu tương ứng trên hệ thống X11 vì các nhà sản xuất đôi khi điều chỉnh màu X11 cho phần cứng cụ thể của họ.
...
- {{cssxref("&lt;color&gt;")}}: kiểu dữ liệu mà `<named-color>` là một thành phần.
