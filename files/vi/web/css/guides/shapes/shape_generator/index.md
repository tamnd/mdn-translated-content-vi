...
Công cụ tạo shape này có thể được dùng để xác định tọa độ và cú pháp cho các tính năng sử dụng {{cssxref("basic-shape")}}

- Thuộc tính [`coords`](/en-US/docs/Web/HTML/Reference/Elements/area#coords) của phần tử {{HTMLElement("area")}}
- Các hàm CSS {{cssxref("basic-shape/inset()")}}, {{cssxref("basic-shape/xywh()")}}, {{cssxref("basic-shape/rect()")}}, {{cssxref("basic-shape/circle()")}}, {{cssxref("basic-shape/polygon()")}}
- Các phần tử SVG {{SVGElement("rect")}}, {{SVGElement("circle")}}, {{SVGElement("polygon")}}

Công cụ tạo shape giúp bạn trực quan hóa các basic shape, xuất ra tọa độ của từng điểm, được chồng lên hình ảnh của bạn. Tải hình ảnh lên bằng cách kéo thả hoặc nhấp để chọn tệp. Sau đó chọn loại shape bạn muốn tạo từ menu thả xuống. Tùy chọn, bạn cũng có thể điều chỉnh tỷ lệ của canvas bằng thanh trượt. Sau đó nhấp vào các điểm trên hình ảnh để thêm điểm tham chiếu cho shape bạn muốn tạo. Công cụ sẽ xuất ra markup cho giá trị thuộc tính HTML `coords`, hàm CSS basic shape với tọa độ và phần tử SVG.
