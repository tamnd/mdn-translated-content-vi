...
Thuộc tính chỉ đọc **`y2`** của giao diện {{domxref("SVGLinearGradientElement")}} mô tả tọa độ trục y của điểm kết thúc của gradient dưới dạng {{domxref("SVGAnimatedLength")}}. Nó phản ánh giá trị tính toán của thuộc tính {{SVGAttr("y2")}} trên phần tử {{SVGElement("linearGradient")}}.
...
console.dir(y2Gradient1.baseVal.value); // output: 200 (100% của 200)
console.dir(y2Gradient2.baseVal.value); // output: 200 (100% của 200)
```
