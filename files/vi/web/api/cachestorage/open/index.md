...
Phương thức **`open()`** của giao diện {{domxref("CacheStorage")}} trả về một {{jsxref("Promise")}} phân giải thành đối tượng {{domxref("Cache")}} khớp với `cacheName`.
...
> [!NOTE]
> Nếu {{domxref("Cache")}} được chỉ định không tồn tại, một bộ nhớ đệm mới sẽ được tạo với `cacheName` đó và một {{jsxref("Promise")}} phân giải thành đối tượng {{domxref("Cache")}} mới này sẽ được trả về.
...
- `cacheName`
  - : Tên của bộ nhớ đệm mà bạn muốn mở.
...
Ở đây chúng ta chờ một {{domxref("InstallEvent")}} xảy ra, rồi chạy {{domxref("ExtendableEvent.waitUntil","waitUntil()")}} để xử lý quá trình cài đặt cho ứng dụng. Việc này bao gồm gọi `CacheStorage.open()` để tạo một bộ nhớ đệm mới, rồi dùng {{domxref("Cache.addAll()")}} để thêm một loạt tài nguyên vào đó.
