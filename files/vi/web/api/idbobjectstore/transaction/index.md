---
title: "IDBObjectStore: thuộc tính transaction"
...
Thuộc tính chỉ đọc **`transaction`** của giao diện {{domxref("IDBObjectStore")}} trả về đối tượng giao dịch mà object store này thuộc về.
...
Trong đoạn mã sau, chúng ta mở một giao dịch read/write trên cơ sở dữ liệu và thêm một số dữ liệu vào object store bằng `add()`. Sau khi object store được tạo, chúng ta ghi `objectStore.transaction` ra console. Để xem ví dụ hoạt động đầy đủ, hãy tham khảo ứng dụng [To-do Notifications](https://github.com/mdn/dom-examples/tree/main/to-do-notifications) của chúng tôi ([xem ví dụ trực tiếp](https://mdn.github.io/dom-examples/to-do-notifications/)).
