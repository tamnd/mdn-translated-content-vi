---
title: "IDBObjectStore: thuộc tính autoIncrement"
...
Thuộc tính chỉ đọc **`autoIncrement`** của giao diện {{domxref("IDBObjectStore")}} trả về giá trị của cờ tự động tăng cho object store này.
...
Trong đoạn mã sau, chúng ta mở một giao dịch read/write trên cơ sở dữ liệu và thêm một số dữ liệu vào object store bằng `add()`. Sau khi object store được tạo, chúng ta ghi `objectStore.autoIncrement` ra console. Để xem ví dụ hoạt động đầy đủ, hãy tham khảo ứng dụng [To-do Notifications](https://github.com/mdn/dom-examples/tree/main/to-do-notifications) của chúng tôi ([xem ví dụ trực tiếp](https://mdn.github.io/dom-examples/to-do-notifications/)).
