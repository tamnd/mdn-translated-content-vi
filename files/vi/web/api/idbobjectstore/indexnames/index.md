---
title: "IDBObjectStore: thuộc tính indexNames"
...
Thuộc tính chỉ đọc **`indexNames`** của giao diện {{domxref("IDBObjectStore")}} trả về danh sách tên của các [index](/vi/docs/Web/API/IndexedDB_API/Basic_Terminology#index) trên các đối tượng trong object store này.
...
Trong đoạn mã sau, chúng ta mở một giao dịch read/write trên cơ sở dữ liệu và thêm một số dữ liệu vào object store bằng `add()`. Sau khi object store được tạo, chúng ta ghi `objectStore.indexNames` ra console. Để xem ví dụ hoạt động đầy đủ, hãy tham khảo ứng dụng [To-do Notifications](https://github.com/mdn/dom-examples/tree/main/to-do-notifications) của chúng tôi ([xem ví dụ trực tiếp](https://mdn.github.io/dom-examples/to-do-notifications/)).
