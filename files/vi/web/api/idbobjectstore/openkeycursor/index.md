---
title: "IDBObjectStore: phương thức openKeyCursor()"
...
Phương thức **`openKeyCursor()`** của giao diện {{domxref("IDBObjectStore")}} trả về một đối tượng {{domxref("IDBRequest")}} mà kết quả sẽ được đặt thành một {{domxref("IDBCursor")}} có thể được sử dụng để lặp qua các kết quả khớp. Được sử dụng để lặp qua các khóa của một object store bằng con trỏ.
...
- một đối tượng {{domxref("IDBCursor")}} trỏ đến bản ghi đầu tiên khớp với truy vấn đã cho
