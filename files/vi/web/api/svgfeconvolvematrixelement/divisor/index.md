...
Thuộc tính chỉ đọc **`divisor`** của giao diện {{domxref("SVGFEConvolveMatrixElement")}} phản ánh thuộc tính {{SVGAttr("divisor")}} của phần tử {{SVGElement("feConvolveMatrix")}} đã cho.
...
### Truy cập thuộc tính `divisor`

Một bộ lọc tích chập được áp dụng cho hình chữ nhật, và `divisor` được dùng để kiểm soát độ sáng.
