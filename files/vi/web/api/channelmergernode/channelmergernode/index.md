---
title: "ChannelMergerNode: hàm tạo ChannelMergerNode()"
...
Hàm tạo **`ChannelMergerNode()`** tạo một thực thể đối tượng {{domxref("ChannelMergerNode")}} mới.
...
- `options` {{optional_inline}}
  - : Một đối tượng xác định các thuộc tính mà bạn muốn `ChannelMergerNode` có:
    - `numberOfInputs` {{optional_inline}}
      - : Một số xác định số lượng đầu vào mà {{domxref("ChannelMergerNode")}} nên có. Nếu không được chỉ định, giá trị mặc định dùng là 6.
    ...
    - `channelCountMode` {{optional_inline}}
      - : Một chuỗi mô tả cách các kênh phải được khớp giữa
        đầu vào và đầu ra của nút. (Xem {{domxref("AudioNode.channelCountMode")}} để biết thêm
        thông tin, bao gồm cả các giá trị mặc định.)
    - `channelInterpretation` {{optional_inline}}
      - : Một chuỗi mô tả ý nghĩa của các kênh.
        Cách diễn giải này sẽ xác định cách quá trình
        [up-mixing và down-mixing](/en-US/docs/Web/API/Web_Audio_API/Basic_concepts_behind_Web_Audio_API#up-mixing_and_down-mixing) diễn ra.
...
Một thực thể đối tượng {{domxref("ChannelMergerNode")}} mới.

### Ngoại lệ

- `InvalidStateError` {{domxref("DOMException")}}
  - : Được ném ra nếu một tùy chọn như `channelCount` hoặc `channelCountMode` được cung cấp giá trị không hợp lệ.
