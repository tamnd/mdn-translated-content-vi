...
Sự kiện `error` được kích hoạt khi yêu cầu gặp lỗi.
...
Bạn có thể dùng sự kiện `error` để phát hiện sự cố khi tải lên. Để xem ví dụ mã đầy đủ tải lên tệp và hiển thị thanh tiến trình, hãy xem trang {{domxref("XMLHttpRequestUpload")}} chính.

```js
// Trong trường hợp lỗi, chúng ta ẩn thanh tiến trình
...
- Các sự kiện liên quan: {{domxref("XMLHttpRequestEventTarget/loadstart_event", "loadstart")}}, {{domxref("XMLHttpRequestEventTarget/load_event", "load")}}, {{domxref("XMLHttpRequestEventTarget/progress_event", "progress")}}, {{domxref("XMLHttpRequestEventTarget/loadend_event", "loadend")}}, {{domxref("XMLHttpRequestEventTarget/abort_event", "abort")}}
