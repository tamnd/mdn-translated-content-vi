...
Thuộc tính chỉ đọc **`height`** của giao diện {{domxref("SVGForeignObjectElement")}} mô tả chiều cao của phần tử `<foreignObject>`. Nó phản ánh giá trị tính toán của thuộc tính {{SVGAttr("height")}} trên phần tử {{SVGElement("foreignObject")}}.
...
Chúng ta có thể truy cập các giá trị tính toán của thuộc tính `height`:
...
console.dir(heightObject2.baseVal.value); // output: 20 (10% của 200)
```
