...
Thuộc tính chỉ đọc **`width`** của giao diện {{domxref("SVGForeignObjectElement")}} mô tả chiều rộng của phần tử `<foreignObject>`. Nó phản ánh giá trị tính toán của thuộc tính {{SVGAttr("width")}} trên phần tử {{SVGElement("foreignObject")}}.
