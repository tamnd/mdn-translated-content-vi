...
Nếu {{htmlelement("area")}} là một liên kết (chứa thuộc tính {{DOMxRef("HTMLAreaElement.href", "href")}}), giá trị thuộc tính `alt` nên là một chuỗi không rỗng giving nhãn cho liên kết sẽ phù hợp nếu hình ảnh không khả dụng. Thuộc tính `alt` cho một `<area>` là liên kết chỉ có thể để trống nếu có một phần tử `<area>` khác trong cùng `<map>` trỏ đến cùng tài nguyên và có thuộc tính `alt` không trống.
