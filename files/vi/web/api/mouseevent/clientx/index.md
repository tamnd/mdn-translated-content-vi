...
Thuộc tính chỉ đọc **`clientX`** của giao diện {{domxref("MouseEvent")}} cung cấp tọa độ ngang trong {{glossary("viewport")}} của ứng dụng nơi sự kiện xảy ra (ngược lại với tọa độ trong trang).

Ví dụ: nhấp vào cạnh trái của khung nhìn sẽ luôn dẫn đến sự kiện chuột có giá trị `clientX` là `0`, bất kể trang có được cuộn theo chiều ngang hay không.
