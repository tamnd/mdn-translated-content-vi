---
title: "HTMLElement: sự kiện drop"
short-title: drop
...
Sự kiện **`drop`** được kích hoạt khi một phần tử hoặc vùng văn bản được thả vào một vùng thả hợp lệ. Để đảm bảo sự kiện `drop` luôn kích hoạt như mong đợi, bạn nên luôn gọi [`preventDefault()`](/en-US/docs/Web/API/Event/preventDefault) trong phần xử lý sự kiện [`dragover`](/en-US/docs/Web/API/HTMLElement/dragover_event).
...
Để xem ví dụ đầy đủ hơn về kéo và thả, hãy xem trang dành cho sự kiện [`drag`](/en-US/docs/Web/API/HTMLElement/drag_event).
