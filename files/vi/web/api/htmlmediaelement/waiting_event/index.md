---
title: "HTMLMediaElement: sự kiện waiting"
short-title: waiting
...
Sự kiện `waiting` được kích hoạt khi quá trình phát lại đã dừng do thiếu dữ liệu tạm thời.
...
Các ví dụ này thêm trình xử lý sự kiện cho sự kiện `waiting` của HTMLMediaElement, sau đó đăng thông báo khi trình xử lý sự kiện đó phản ứng với việc kích hoạt sự kiện.
...
Sử dụng thuộc tính xử lý sự kiện `onwaiting`:
