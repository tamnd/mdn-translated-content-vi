---
title: "HTMLMediaElement: sự kiện playing"
short-title: playing
...
Sự kiện `playing` được kích hoạt sau khi quá trình phát lại được bắt đầu lần đầu tiên và bất cứ khi nào nó được khởi động lại. Ví dụ: nó được kích hoạt khi quá trình phát lại tiếp tục sau khi bị tạm dừng hoặc bị trì hoãn do thiếu dữ liệu.
...
Các ví dụ này thêm trình xử lý sự kiện cho sự kiện `playing` của HTMLMediaElement, sau đó đăng thông báo khi trình xử lý sự kiện đó phản ứng với việc kích hoạt sự kiện.
...
Sử dụng thuộc tính xử lý sự kiện `onplaying`:
