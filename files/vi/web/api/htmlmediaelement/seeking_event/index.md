---
title: "HTMLMediaElement: sự kiện seeking"
short-title: seeking
...
Sự kiện `seeking` được kích hoạt khi hoạt động tìm kiếm bắt đầu, nghĩa là thuộc tính Boolean `seeking` đã thay đổi thành `true` và phương tiện truyền thông đang tìm kiếm một vị trí mới.
...
Các ví dụ này thêm trình xử lý sự kiện cho sự kiện `seeking` của HTMLMediaElement, sau đó đăng thông báo khi trình xử lý sự kiện đó phản ứng với việc kích hoạt sự kiện.
...
Sử dụng thuộc tính xử lý sự kiện `onseeking`:
