---
title: "HTMLMediaElement: sự kiện volumechange"
...
Sự kiện `volumechange` được kích hoạt khi thuộc tính {{domxref("HTMLMediaElement.volume", "volume")}} hoặc thuộc tính {{domxref("HTMLMediaElement.muted", "muted")}} đã thay đổi.
...
Các ví dụ này thêm trình xử lý sự kiện cho sự kiện `volumechange` của HTMLMediaElement, sau đó đăng thông báo khi trình xử lý sự kiện đó phản ứng với việc kích hoạt sự kiện.
...
Sử dụng thuộc tính xử lý sự kiện `onvolumechange`:
