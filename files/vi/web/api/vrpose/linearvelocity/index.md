...
Thuộc tính chỉ đọc **`linearVelocity`** của giao diện {{domxref("VRPose")}} trả về mảng đại diện cho vectơ vận tốc tuyến tính của {{domxref("VRDisplay")}} tại dấu thời gian hiện tại, tính bằng mét trên giây.
...
Nói cách khác, vận tốc hiện tại mà cảm biến đang di chuyển dọc theo các trục `x`, `y`, và `z`.

## Giá trị

Một {{jsxref("Float32Array")}}, hoặc `null` nếu cảm biến VR không thể cung cấp dữ liệu vận tốc tuyến tính.
