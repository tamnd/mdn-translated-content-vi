...
Thuộc tính chỉ đọc **`linearAcceleration`** của giao diện {{domxref("VRPose")}} trả về mảng đại diện cho vectơ gia tốc tuyến tính của {{domxref("VRDisplay")}} tại dấu thời gian hiện tại, tính bằng mét trên giây bình phương.
...
Một {{jsxref("Float32Array")}}, hoặc `null` nếu cảm biến VR không thể cung cấp dữ liệu gia tốc tuyến tính.
