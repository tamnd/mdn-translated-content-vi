---
title: "IDBTransaction: phương thức abort()"
short-title: abort()
...
Phương thức **`abort()`** của giao diện {{domxref("IDBTransaction")}} hoàn nguyên mọi thay đổi đối với các đối tượng trong cơ sở dữ liệu gắn với transaction này.

Mọi đối tượng {{domxref("IDBRequest")}} đang chờ được tạo trong transaction này sẽ có thuộc tính {{domxref("IDBRequest.error")}} được đặt thành một {{domxref("DOMException")}} `AbortError`.
