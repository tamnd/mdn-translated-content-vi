...
Sự kiện **`mark`** của đối tượng {{domxref("SpeechSynthesisUtterance")}} thuộc [Web Speech API](/en-US/docs/Web/API/Web_Speech_API) được kích hoạt khi yêu cầu phát âm đang được nói đến thẻ SSML "mark" được đặt tên.
...
- {{domxref("SpeechSynthesisEvent.name", "name")}} {{ReadOnlyInline}}
  - : Trả về tên được liên kết với một số loại sự kiện xảy ra khi {{domxref("SpeechSynthesisUtterance.text")}} đang được nói: tên của marker [SSML](https://www.w3.org/TR/speech-synthesis/#S3.3.2) đạt đến trong trường hợp sự kiện `mark`, hoặc loại ranh giới đạt đến trong trường hợp sự kiện {{domxref("SpeechSynthesisUtterance.boundary_event", "boundary")}}.
...
Bạn có thể sử dụng sự kiện `mark` trong phương thức [`addEventListener`](/en-US/docs/Web/API/EventTarget/addEventListener):
...
Hoặc sử dụng thuộc tính trình xử lý sự kiện `onmark`:
