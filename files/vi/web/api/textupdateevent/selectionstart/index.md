...
Thuộc tính chỉ đọc **`TextUpdateEvent.selectionStart`** cho biết vị trí bắt đầu của vùng chọn (hoặc con trỏ) trong nội dung văn bản của vùng có thể chỉnh sửa được gắn với đối tượng {{domxref("EditContext")}}.
...
Ví dụ này cho thấy cách sử dụng thuộc tính `selectionStart` để hiển thị văn bản được chọn trong trình xử lý sự kiện {{domxref("EditContext/textupdate_event", "textupdate")}}.
