...
Phương thức **`WEBGL_multi_draw.multiDrawArraysWEBGL()`** của [WebGL API](/en-US/docs/Web/API/WebGL_API) kết xuất nhiều hình nguyên thủy từ dữ liệu mảng. Phương thức này giống với nhiều lần gọi phương thức [`gl.drawArrays()`](/en-US/docs/Web/API/WebGLRenderingContext/drawArrays).
...
- Nếu `drawCount` hoặc các mục trong `firstsList` và `countsList` là âm, lỗi `gl.INVALID_VALUE` sẽ được ném ra.
