---
title: "WritableStream: phương thức getWriter()"
...
Phương thức **`getWriter()`** của giao diện {{domxref("WritableStream")}} trả về một phiên bản mới của {{domxref("WritableStreamDefaultWriter")}} và khóa luồng tới phiên bản đó.
Trong khi luồng bị khóa, không thể lấy được trình ghi nào khác cho đến khi luồng này được phát hành.
...
const writableStream = new WritableStream(
  {
    ...
    close() {
      const listItem = document.createElement("li");
      listItem.textContent = `[MESSAGE ĐÃ NHẬN] ${result`;
      list.appendChild(listItem);
    },
    abort(err) {
...
Bạn có thể tìm thấy mã đầy đủ trong [Ví dụ về người viết đơn giản](https://mdn.github.io/dom-examples/streams/simple-writer/) của chúng tôi.
