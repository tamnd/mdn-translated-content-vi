...
Thuộc tính chỉ đọc **`dy`** của giao diện {{domxref("SVGFEDropShadowElement")}} phản ánh thuộc tính {{SVGAttr("dy")}} của phần tử {{SVGElement("feDropShadow")}} đã cho.
...
Trong ví dụ này, chúng ta lấy độ dịch chuyển dọc của `<feDropShadow>` bằng cách sử dụng thuộc tính chỉ đọc `dx` của giao diện `SVGFEDropShadowElement`.
