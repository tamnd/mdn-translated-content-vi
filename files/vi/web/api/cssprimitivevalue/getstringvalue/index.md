...
Phương thức **`getStringValue()`** của giao diện {{domxref("CSSPrimitiveValue")}} dùng để lấy giá trị chuỗi. Nếu giá trị CSS này không chứa giá trị chuỗi, một {{domxref("DOMException")}} sẽ được ném ra.
...
| **Kiểu**       | **Mô tả**                                                                      |
| -------------- | ------------------------------------------------------------------------------ |
| `DOMException` | Một `INVALID_ACCESS_ERR` được ném ra nếu giá trị CSS không chứa giá trị chuỗi. |
