...
Thuộc tính chỉ đọc **`length`** của giao diện {{domxref("TextTrackCueList")}} trả về số lượng cue trong danh sách.
...
Thuộc tính {{domxref("TextTrack.cues")}} trả về {{domxref("TextTrackCueList")}} chứa các cue hiện tại cho rãnh đó. Gọi `cues.length` trả về số lượng cue trong danh sách. Sử dụng rãnh WebVTT bên dưới, giá trị của `length` là 5.
