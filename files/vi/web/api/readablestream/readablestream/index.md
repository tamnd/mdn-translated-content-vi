...
Hàm tạo **`ReadableStream()`** tạo và trả về một đối tượng luồng có thể đọc được từ các trình xử lý đã cho.

Lưu ý rằng mặc dù tất cả các tham số đều là tùy chọn về mặt kỹ thuật, việc bỏ qua `underlyingSource` sẽ dẫn đến luồng không có nguồn và không thể đọc được (người đọc sẽ trả lại lời hứa sẽ không bao giờ được giải quyết).
  ...
  - : An object containing methods and properties that define how the constructed stream instance will behave.
    ...
    - `pull` (controller) {{optional_inline}}
      - : This method, also defined by the developer, will be called repeatedly when the
        ...
        errored. The `con troller` parameter passed to this method is a
    ...
    - `type` {{optional_inline}}
      - : This property controls what type of readable stream is being dealt with. If it
        is included with a value set to `"byte"`, the passed controller object
    ...
    - `autoAllocateChunkSize`
      - : Set this property to a positive integer value to turn on the stream's auto-allocation feature.
...
- `queuingStrategy` {{optional_inline}}
  - : An object that optionally defines a queuing strategy for the stream. This takes two
    ...
    > for this object value. If no `queu ingStrategy` is supplied, the default
    > used is the same as a `CountQueuingStrargety` with a high water mark of
    > 1\.
...
- {{jsxref("RangeError")}}
  - : Thrown if the supplied type value is neither `"byte"` nor `undefined`.
