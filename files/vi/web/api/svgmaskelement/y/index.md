...
Thuộc tính chỉ đọc **`y`** của giao diện {{domxref("SVGMaskElement")}} trả về một đối tượng {{domxref("SVGAnimatedLength")}} chứa giá trị của thuộc tính {{SVGattr("y")}} của {{SVGElement("mask")}}. Nó đại diện cho tọa độ trục y của góc _trên bên trái_ của vùng che.
...
Một đối tượng {{domxref("SVGAnimatedLength")}}. Thuộc tính `baseVal` của đối tượng này trả về một {{domxref("SVGLength")}}, giá trị của nó trả về giá trị `y`.
