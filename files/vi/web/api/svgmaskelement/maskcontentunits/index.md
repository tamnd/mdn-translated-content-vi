...
Thuộc tính chỉ đọc **`maskContentUnits`** của giao diện {{domxref("SVGMaskElement")}} phản ánh thuộc tính {{SVGAttr("maskContentUnits")}}. Nó cho biết hệ tọa độ nào sẽ được sử dụng cho nội dung của phần tử {{SVGElement("mask")}}.
...
- `1` (`SVG_UNIT_TYPE_USERSPACEONUSE`)
  - : Tương ứng với giá trị `userSpaceOnUse` cho thuộc tính {{SVGAttr("maskContentUnits")}}, nghĩa là tất cả tọa độ bên trong phần tử đều tham chiếu đến hệ tọa độ người dùng được xác định khi mặt nạ được tạo. Đây là giá trị mặc định.
