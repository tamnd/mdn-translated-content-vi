...
Thuộc tính chỉ đọc **`width`** của giao diện {{domxref("SVGMaskElement")}} trả về một đối tượng {{domxref("SVGAnimatedLength")}} chứa giá trị của thuộc tính {{SVGattr("width")}} của {{SVGElement("mask")}}.
...
Một đối tượng {{domxref("SVGAnimatedLength")}}. Thuộc tính `baseVal` của đối tượng này trả về một {{domxref("SVGLength")}}, giá trị của nó trả về giá trị `width`.
