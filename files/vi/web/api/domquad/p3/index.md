...
Thuộc tính **`p3`** của giao diện **`DOMQuad`** giữ đối tượng {{domxref("DOMPoint")}} đại diện cho một trong bốn góc của `DOMQuad`. Khi được tạo từ {{domxref("DOMQuad.fromRect_static", "DOMQuad.fromRect()")}}, đây là điểm (x + width, y + height).
...
- Các thuộc tính `DOMPoint` khác: {{domxref("DOMQuad.p1", "p1")}},
  {{domxref("DOMQuad.p2", "p2")}}, và {{domxref("DOMQuad.p4", "p4")}}.
