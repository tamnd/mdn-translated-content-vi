...
Phương thức **`write()`** của giao diện {{domxref("FileSystemSyncAccessHandle")}} ghi nội dung của một buffer được chỉ định vào tệp liên kết với handle, tùy chọn tại một offset nhất định.
...
- `buffer`
  - : Một {{jsxref("ArrayBuffer")}} hoặc `ArrayBufferView` (chẳng hạn {{jsxref("DataView")}}) đại diện cho buffer cần ghi vào tệp.
...
> [!NOTE]
> Bạn không thể thao tác trực tiếp nội dung của `ArrayBuffer`. Thay vào đó, hãy tạo một đối tượng mảng có kiểu như {{jsxref("Int8Array")}} hoặc đối tượng {{jsxref("DataView")}} đại diện cho buffer ở định dạng cụ thể.
...
- {{domxref("QuotaExceededError")}}
  - : Được ném ra nếu dung lượng dữ liệu tăng thêm vượt quá [hạn ngạch lưu trữ](/en-US/docs/Web/API/Storage_API/Storage_quotas_and_eviction_criteria) của trình duyệt.
- {{jsxref("TypeError")}}
