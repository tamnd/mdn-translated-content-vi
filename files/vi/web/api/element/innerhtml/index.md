...
Thuộc tính **`innerHTML`** của giao diện {{domxref("Element")}} gets or sets the HTML or XML markup contained within the element, omitting any {{glossary("shadow tree", "shadow roots")}} in both cases.
...
Getting the property trả về a string containing the HTML serialization of the element's descendants.
...
Trong ví dụ này we'll replace an element's DOM by assigning HTML to the element's `innerHTML` property.
