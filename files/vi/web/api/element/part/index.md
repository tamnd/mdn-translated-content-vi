...
The read-only **`part`** của giao diện {{domxref("Element")}} chứa a {{domxref("DOMTokenList")}} object representing the part identifier(s) of the element. It reflects the element's [`part`](/en-US/docs/Web/HTML/Reference/Global_attributes/part) content attribute. These can be used to style parts of a shadow DOM, via the {{cssxref("::part")}} pseudo-element.

## Giá trị

Một {{domxref("DOMTokenList")}} object. If the `part` attribute is not set or empty, it trả về an empty `DOMTokenList`, i.e., a `DOMTokenList` with the `length` property equal to `0`.
...
Các giá trị sau excerpt is from our [shadow-part](https://mdn.github.io/web-components-examples/shadow-part/)
