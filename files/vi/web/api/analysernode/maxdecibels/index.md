---
title: "AnalyserNode: thuộc tính maxDecibels"
...
Thuộc tính **`maxDecibels`** của giao diện {{domxref("AnalyserNode")}} là một giá trị `double` biểu diễn giá trị công suất lớn nhất trong khoảng tỉ lệ cho dữ liệu phân tích FFT, để chuyển sang giá trị byte không dấu. Nói cách khác, thuộc tính này xác định giá trị lớn nhất của dải kết quả khi dùng `getByteFrequencyData()`.

## Giá trị

Một giá trị `double`, biểu diễn giá trị [decibel](https://en.wikipedia.org/wiki/Decibel) lớn nhất để tỉ lệ hóa dữ liệu phân tích FFT, trong đó `0` dB là âm thanh lớn nhất có thể, `-10` dB là một phần mười của mức đó, v.v. Giá trị mặc định là `-30` dB.
...
- `IndexSizeError` {{domxref("DOMException")}}
  - : Được ném ra nếu đặt một giá trị nhỏ hơn hoặc bằng `AnalyserNode.minDecibels`.
