...
Sự kiện `redraw` được gửi đến đối tượng `XRCylinderLayer` khi các tài nguyên cơ bản của lớp bị mất hoặc khi XR Compositor không thể chiếu lại lớp. Nếu sự kiện này được gửi, các tác giả nên vẽ lại nội dung của lớp trong khung hình hoạt ảnh XR tiếp theo.

Xem thêm thuộc tính {{domxref("XRCompositionLayer.needsRedraw")}} cũng có sẵn cho các đối tượng `XRCylinderLayer` thông qua kế thừa từ {{domxref("XRCompositionLayer")}}.
