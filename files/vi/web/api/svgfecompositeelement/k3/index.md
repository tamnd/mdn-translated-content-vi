...
Thuộc tính chỉ đọc **`k3`** của giao diện {{domxref("SVGFECompositeElement")}} phản ánh thuộc tính {{SVGAttr("k3")}} của phần tử {{SVGElement("feComposite")}} đã cho.
...
Trong ví dụ này, hai phần tử {{SVGElement("feComposite")}} được định nghĩa trong một bộ lọc, mỗi phần tử có thuộc tính `k3` khác nhau.
...
Chúng ta có thể truy cập thuộc tính `k3`:
