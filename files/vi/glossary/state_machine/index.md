...
- Máy trạng thái hữu hạn xác định (Deterministic finite state machine)
  - : Loại này chỉ cho phép một chuyển tiếp có thể duy nhất cho bất kỳ đầu vào được phép nào. Điều này giống như một {{Glossary("statement", "câu lệnh")}} "if" vì `if x then doThis else doThat` là không thể. Máy tính phải thực hiện _một_ trong hai tùy chọn.
