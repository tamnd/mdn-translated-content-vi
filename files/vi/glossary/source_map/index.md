...
Code được thực thi bởi trình duyệt thường được chuyển đổi theo một cách nào đó từ nguồn gốc ban đầu được tạo ra bởi nhà phát triển. Có một số lý do cho điều này:
...
Trong các tình huống này, việc gỡ lỗi nguồn gốc trực quan hơn so với nguồn ở trạng thái đã chuyển đổi mà trình duyệt đã tải xuống. Trình duyệt phát hiện source map thông qua HTTP header {{HTTPHeader("SourceMap")}} cho một tài nguyên, hoặc chú thích `sourceMappingURL` trong code đã tạo ra.
...
- Response header HTTP {{HTTPHeader("SourceMap")}}
